/**
 * MOTOR DE EXAMEN CENTRALIZADO
 * Este archivo gestiona dinámicamente cualquier materia
 * importando su banco de preguntas desde la carpeta correspondiente en la raíz.
 */
...
const etiquetasMateria = {
  ...
  tecnologia: {
    ...
    temas: {
      producto: "Productos y Necesidades",
      procesos: "Procesos y Materiales",
...
async function cargarBancoYIniciar() {
  try {
    ...
    contenedor.innerHTML = `<div class="col-12 p-5 text-center alert-error-tech">
            Error: No se pudo conectar con el banco de preguntas de ${materiaActiva}.
        </div>`;
  }
}
...
function inicializarExamen(banco) {
  ...
  let bancoFiltrado = banco.filter((p) => {
    ...
    if (temaActual === "mix") return nivelCoincide;
                        ...
                        ${p.opciones
                          .map(
                            ...
                            <div class="form-check custom-option mb-3">
                                ...
                                <label class="form-check-label px-3 py-2 w-100" for="q${index}o${i}" style="border: 1px solid #ccc; border-radius: 4px; cursor: pointer;">
                                    ${escaparHtml(opc)}
                                </label>
                            </div>
                        `,
